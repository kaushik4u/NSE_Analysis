...
def get_3months(ticker, proxy):
    url = (
        "https://www.nseindia.com/live_market/dynaContent/live_watch/get_quote/getHistoricalData.jsp?symbol="
        + ticker
        + "&series=EQ&fromDate=undefined&toDate=undefined&datePeriod=3months"
    )
    if proxy:
        req = requests.get(
            url, headers={"User-Agent": "Chrome Browser"}, proxies=proxyDict
        )
    else:
...
def fetch_bhavcopy(datestring, proxy):
    bhavurl = (
        "http://www.nseindia.com/ArchieveSearch?h_filetype=eqbhav&date="
        + datestring
        + "&section=EQ"
    )
    ...
    if proxy:
        req = requests.get(
            bhavurl, headers={"User-Agent": "Chrome Browser"}, proxies=proxyDict
        )
    else:
    ...
    if link != None:
        ...
        if proxy:
            req = requests.get(
                fileurl, headers={"User-Agent": "Chrome Browser"}, proxies=proxyDict
            )
        else:
    ...
    for file in os.listdir(csvDir):
        if file.endswith(".csv"):
            ...
            for chunk in pd.read_csv(csvFile, chunksize=4):
                ...
                chunk.to_sql(
                    name="EOD_data", con=connection, if_exists="append", index=False
